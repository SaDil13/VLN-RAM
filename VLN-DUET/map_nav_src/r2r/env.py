...
class R2RNavBatch(object):
    ...
    def _get_obs(self, aug=False):
        obs = []
        for i, (feature, state) in enumerate(self.env.getStates()):
            item = self.batch[i]
            base_view_id = state.viewIndex

            if isinstance(item['path_id'], str) and '_rwt_' in item['path_id']:
                feature = self.env.feat_db.get_image_feature(state.scanId, state.location.viewpointId, sd_path_id=item['path_id'])
    ...
    def reset(self, aug=False, **kwargs):
    ...
    def _eval_item(self, scan, pred_path, gt_path):
        ...
        nav_success = scores['success']
        # if scores['success'] == 0:
        #     nav_success = 'nav_failed'
        # else:
        #     nav_success = 'nav_success'


        return scores, nav_success
    ...
    def eval_metrics(self, preds):
        ...
        for item in preds:
            ...
            traj_scores, nav_success_item = self._eval_item(scan, traj, gt_traj)
            nav_success[str(instr_id)] = nav_success_item
        ...
        return avg_metrics, metrics, nav_success
